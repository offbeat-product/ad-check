...
export default function LegalRegulationTemplate({ initialData, onChange }: Props) {
  const [activePresets, setActivePresets] = useState<string[]>(initialData?.presets || ["pharmaceutical", "fair_trade"]);
  const [customRules, setCustomRules] = useState(initialData?.custom_rules || "");
  const [industryCustom, setIndustryCustom] = useState("");

  const buildAndNotify = (presets: string[], custom: string, industryText: string) => {
    const presetContents = presets
      .filter(id => id !== "industry_custom")
      .map(id => PRESETS.find(p => p.id === id)?.content || "")
      .filter(Boolean)
      .join("\n\n");

    const fullContent = [presetContents, industryText, custom].filter(Boolean).join("\n\n");
  ...
  const togglePreset = (id: string) => {
    const next = activePresets.includes(id) ? activePresets.filter(p => p !== id) : [...activePresets, id];
  ...
  const handleCustomChange = (text: string) => {
    setCustomRules(text);
    buildAndNotify(activePresets, text, industryCustom);
  };

  const handleIndustryChange = (text: string) => {
    setIndustryCustom(text);
    buildAndNotify(activePresets, customRules, text);
  };

  const activeCount = activePresets.filter(id => id !== "industry_custom").length;

  return (
    <div className="space-y-4">
      <div>
        <h4 className="text-xs font-semibold text-primary border-b border-border pb-1 mb-2">ステップ1: 法令プリセット選択</h4>
              ...
              <div key={preset.id}>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <Switch checked={isActive} onCheckedChange={() => togglePreset(preset.id)} />
                  </div>
                </div>
                {isIndustry && isActive && (
        ...
        {activeCount > 0 && (
          <p className="text-[10px] text-status-ok mt-2 font-medium">✅ {activeCount}カテゴリの法令ルールが適用されます</p>
        )}
      </div>
