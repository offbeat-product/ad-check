...
import { ClipboardList, ListChecks, Palette, Scale, Smartphone, FileEdit } from "lucide-react";
...
export default function ReferenceMaterialsSection({ projectId, productId, productName, projectName }: Props) {
  ...
  const totalRegistered = [...productMaterials, ...projectMaterials].filter(m => m.is_active).length;
