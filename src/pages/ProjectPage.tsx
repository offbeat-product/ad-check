...
import type { Project, Product, Client, ProjectFile } from "@/lib/db-types";
...
import {
  Upload, FileText, Image, Film, MessageCircle, MoreVertical, Plus,
} from "lucide-react";
...
export default function ProjectPage() {
  ...
  const [uploadModal, setUploadModal] = useState<string | null>(null); // process_type
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: prod }, { data: fileData }] = await Promise.all([
      supabase.from("products").select("*").eq("id", (proj as any).product_id).single(),
    ...
    // Fetch check results for files that have them
    const fileIds = ((fileData as any) || []).filter((f: any) => f.check_result_id).map((f: any) => f.check_result_id);
    if (fileIds.length > 0) {
      const { data: results } = await supabase.from("check_results").select("id, overall_status, ng_count, warning_count").in("id", fileIds);
  ...
  useEffect(() => {
    ...
    supabase.from("comments").select("check_result_id").in("check_result_id", fileCheckIds).then(({ data }) => {
                    ...
                    {sectionFiles.length === 0 ? (
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                        {sectionFiles.map((file) => {
                          ...
                          const st = FILE_STATUS_CONFIG[file.status] || FILE_STATUS_CONFIG.uploaded;
                          const cc = file.check_result_id ? (commentCounts[file.check_result_id] || 0) : 0;
                          const isImage = file.file_type === "image";
                          const versions = files.filter(f => f.parent_file_id === file.id);
                          const versionLabel = file.parent_file_id ? `v${file.version_number}` : versions.length > 0 ? "v1" : null;

                          return (
                            <button
                              key={file.id}
                              onClick={() => navigate(`/project/${id}/file/${file.id}`)}
          ...
          <div className="space-y-4">
            {uploadModal === "script" && (
              <div className="flex gap-2">
                <Button size="sm" variant={useTextInput ? "outline" : "default"} onClick={() => setUseTextInput(false)} className="text-xs">
                  ファイル選択
                </Button>
                <Button size="sm" variant={useTextInput ? "default" : "outline"} onClick={() => setUseTextInput(true)} className="text-xs">
                  テキスト直接入力
                </Button>
              </div>
            )}
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (f) { setSelectedFile(f); if (!uploadFileName) setUploadFileName(f.name); }
                  }}
                />
              </div>
            )}
...
function CheckHistory({ projectId, files, checkResults }: { projectId: string; files: ProjectFile[]; checkResults: Record<string, any> }) {
  const navigate = useNavigate();
