...
export function CLSubmitDialog({ projectId, projectName, open, onClose, onSuccess }: CLSubmitDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    const patch: ProjectFileUpdate = { submission_type: "client", status: "client_review" };
    const { error } = await supabase.from("project_files").update(patch).in("id", ids);
    setSubmitting(false);
    if (error) {
      toast({ title: "提出失敗", description: error.message, variant: "destructive" });
      return;
    }
    toast({ title: `${ids.length}件をクライアント提出しました` });
    onSuccess();
    onClose();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>クライアントに提出</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            案件「{projectName}」の以下のファイルをクライアント提出します。提出するファイルを選択してください。
          </p>
          {loading ? (
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={() => void handleSubmit()} disabled={selectedIds.size === 0 || submitting}>
            {submitting ? "処理中..." : `${selectedIds.size}件を提出 →`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
